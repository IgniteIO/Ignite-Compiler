...
class SafePHP {
    ...
    public function evaluate($code) {
        ...
        $buffer = htmlentities(ob_get_contents());
        ob_end_clean();

        $this->output = array('output' => $buffer);
        ...
        if (error_get_last())
            $this->output['error'] = error_get_last();
        if(!empty($this->errors))
            $this->output['safe_errors'] = $this->errors;
     ...
     * @param array $errors
     */
    public function html_errors($errors = null) {
        if($errors) {
            $this->errors = $errors;
            $this->errorsHTML = '<dl>';
     ...
     * @param array $errors
     */
    public function text_errors($errors = null) {
        if($errors) {
            $this->errors = $errors;
            foreach ($this->errors as $error) {
    ...
    public static function fatal_error_handler() {
        ...
        if ($error !== NULL) {
            ...
            unset($error['file']);
            $format = "<dl>
	<dt>Line " . $error['line'] . "</dt>
	<dd>" . $error['message'] . "</dd>
</dl>";
            echo $format;
            $nice_error = ob_get_contents();

            ob_end_clean();

            exit('<BREAK>' . json_encode(array('status' => 'success', 'output' => null, 'errors' => $nice_error, 'earray' => $error)));
        }
    }
     ...
     * @return string
     */
    private static function safe_file_get_contents($url) {
